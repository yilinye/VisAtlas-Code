<!--
 * @Author: Qing Shi
 * @LastEditTime: 2022-04-12 21:32:15
 * @Knowledge: 
 * @Description: 
 * @Attention: 
-->
...
export default {
  ...
  data () {
    return {
      ...
      hs: "calc(16.6% - 11px)",
      wd: "calc(50% - 7px)",
      hisHeight: document.getElementById(('His' + hisType)).offsetHeight,
      hisWidth: document.getElementById(('His' + hisType)).offsetWidth
    };
  },
  inject: ['fileP'],
  methods: {
    ...
    RectBrush (hisData, hisType) {
      ...
      const brush = d3.brush()
        .extent([
          [30, 15],
          [this.hisWidth - 5, this.hisHeight - 10]
        ])
      // .on('brush', brushing)
      // .on('end', brushed);
